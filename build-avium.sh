...
LUNCH_TARGET="lineage_audi-bp4a-userdebug"
DEVICE="audi"
LOG="build.log"

START_TIME=$(date +%s)
LOG_TAG="Avium UI | audi"
...
echo "════════════════════════════════════"
echo "  Avium UI Build — OnePlus ACE 3V"
echo "════════════════════════════════════"
...
cat > .repo/local_manifests/avium_audi.xml << 'LOCALMANIFEST'
<?xml version="1.0" encoding="UTF-8"?>
<manifest>
  <remote name="firefly"
          fetch="https://github.com/NaHCO3-Firefly" />
...
git clone "https://${GH_TOKEN}@github.com/NaHCO3-Firefly/lineage-priv.git" -b main vendor/lineage-priv
...
🕛 Time: $(elapsed)"
    ...
    mapfile -t ZIP_CANDIDATES < <(compgen -G "${OUT_DIR}/AviumUI-*.zip"; compgen -G "${OUT_DIR}/*${DEVICE}*.zip")
